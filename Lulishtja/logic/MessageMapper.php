...
class MessageMapper extends DatabaseConfig{
    ...
    public function getMessagesbyIds($id){
    ...
    public function setAsRead($id){
        $this->query="update messges set is_read=1 where id=:id";
        $statement = $this->connection->prepare($this->query);
        $statement -> bindParam(":id",$id);
        $statement -> execute();
    }
    public function setAsUnread($id){
        $this->query="update messges set is_read=0 where id=:id";
        $statement = $this->connection->prepare($this->query);
        $statement -> bindParam(":id",$id);
        $statement -> execute();
    }
    ...
    public function updatetext($id, $text){
        $this->query="update contact set text=:newtext where id=:id";
        $statement=$this->connection->prepare($this->query);
        $statement -> bindParam(":id",$id);
